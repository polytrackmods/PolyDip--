...
class Stopwatch {
        ...
        setTimer(q) {
            this.ui = q;
            this.ui.style.color = this.running ? "#00ff00" : "gray";
        }
        start() {
            console.log("starting")
            this.ui ? this.ui.style.color = "#00ff00" : null;
        ...
        stop() {
            ...
            this.running = false;
            this.ui ? this.ui.style.color = "gray" : null;
        }
        clear() {
            this.stop();
            this.ui ? this.ui.textContent = "0" : null;
...
class pdipMod extends PolyMod { 
    pbFromServer = async function(playerId) {
        let pbJson = await fetch(`https://polydip.ireo.xyz/pb/${playerId}`).then((r) => r.json());
        return pbJson;
    };
    pbToServer = function(playerId, playerName, pbHeight) {
        fetch("https://polydip.ireo.xyz/updatepb", {
            method: "PUT",
            headers: {
    ...
    updateUserPB = function(newToken) {
        ...
        this.pbFromServer(newToken).then((r) => {
                if(r.error) {
                    ...
                    console.log(`Got PB of ${r.height} for user ${newToken}.`)
    ...
    fetchWR = async function() {
        let pbJson = await fetch("https://polydip.ireo.xyz/wr").then((r) => r.json());
        return pbJson;
    };
    checkWR = function() {
        this.fetchWR().then((r) => {
    ...
    floorPopup = function(floor_num) {
        if (this.pml.getSetting("popupSetting") === "true") {
            const color = this.popupInfo[floor_num - 1][1]
            const desc = this.popupInfo[floor_num - 1][0]
    ...
    createPolyDipUI = function(pb, player_name) {
  ...
  init = function(polyModLoader) {
    ...
    polyModLoader.registerFuncMixin("qL", MixinType.INSERT, `{`, `ActivePolyModLoader.getMod("pdip").trackId = OR(this, TR, "f");console.log(OR(this, TR, "f"));`)

    //main ui from here
    polyModLoader.registerFuncMixin("uI", MixinType.INSERT, 'if (e) {', `
        ...
        if(modInst.trackId === "8cbcb138be4608cbc2b12f956dfadcf66ebfcf013788f0f34abc2603909fde50"){
            if (!modInst.stopWatch.running) {modInst.timeLength = localStorage.getItem(\`pdip_timer_\${modInst.tokenHash}\`) ?? 0;}
            modInst.createPolyDipUI(modInst.pbHeight, modInst.playerName);
        };
    `);

    polyModLoader.registerClassMixin("MD.prototype", "getCurrentUserProfile", MixinType.INSERT, '{', 'ActivePolyModLoader.getMod("pdip").playerName = fL(this, hL, "f").nickname;ActivePolyModLoader.getMod("pdip").tokenHash = fL(this, hL, "f").tokenHash;');
    polyModLoader.registerClassMixin("MD.prototype", "setProfileSlot", MixinType.INSERT, 'pL(this, hL, fL(this, sL, "m", uL).call(this, fL(this, cL, "f")), "f"),', 'ActivePolyModLoader.getMod("pdip").updateUserPB(fL(this, hL, "f").tokenHash),');
    polyModLoader.registerClassMixin("UE.prototype", "refresh", MixinType.INSERT, "{", "ActivePolyModLoader.getMod('pdip').trackId = null;if (ActivePolyModLoader.getMod('pdip').stopWatch.ui) {ActivePolyModLoader.getMod('pdip').stopWatch.stop();}");
      
    this.car = null;
    this.spectator = null;
    polyModLoader.registerFuncMixin("vI", MixinType.INSERT, `yP(this, eP, "f").setColors(n.carColors),`, `ActivePolyModLoader.getMod("${this.modID}").car = yP(this, eP, "f"),`);
    polyModLoader.registerFuncMixin("gI", MixinType.INSERT, `var e;`, `ActivePolyModLoader.getMod("pdip").isInPB = false;`);
    polyModLoader.registerClassMixin("tC.prototype", "addToggleListener", MixinType.INSERT, `a_(this, QT, "f").push(e)`, `,ActivePolyModLoader.getMod("${this.modID}").spectator = this;`);
    ...
    polyModLoader.registerClassMixin("Zk.prototype", "dispose", MixinType.INSERT, `{`, `
    const modInst = ActivePolyModLoader.getMod("pdip");
